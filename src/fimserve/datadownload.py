...
def download_data(huc_number, base_dir):
    ...
    print(
        f"Copied the {hydrotable_path} to {fim_inputs_path} as fim_inputs.csv."
    )
...
def uniqueFID(hydrotable, fid_dir, stream_order=None):
    ...
    if stream_order:
        if isinstance(stream_order, str) and ('>=' in stream_order or '<=' in stream_order or '>' in stream_order or '<' in stream_order):
            ...
            hydrotable_df_filtered = hydrotable_df[hydrotable_df['order_'].isin(stream_order)]
        else:
            raise ValueError(
